...
void connect_serial(serialib* sPtr, const char* port, unsigned long int baudRate)
{
    char errorOpening = sPtr->openDevice(port, baudRate);
...
void send_string_ack(serialib* sPtr, const char* str)
{
    cout << "QUEUED: " << str << endl;

    // send string bytes:
    for (int i = 0; str[i] != '\0'; i++)
        send_byte_ack(sPtr, str[i]);
    ...
    while (!sPtr->available());
    sPtr->readString(str1, '$', 2, 0);
    cout << endl;
}

void send_byte_ack(serialib* sPtr, char byte)
{
    // send byte:
    sPtr->writeChar(byte);
