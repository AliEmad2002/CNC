...
void GPIO_voidSetPinMode(GPIO_PortName_t port, u8 pin, GPIO_Mode_t mode)
{
	...
	if (pin > 7)
	{
		...
		crIndex = 0;
	if (mode > GPIO_Mode_AF_OpenDrain)
	{
		mode -= 4;
		EDT_REG(GPIO[port]->CR[crIndex], 4*pin, 0, 2);
	}
	EDT_REG(GPIO[port]->CR[crIndex], 4*pin+2, mode, 2);
	#endif
}
...
void GPIO_voidSetPinInputPullUp(GPIO_PortName_t port, u8 pin)
{
	...
	GPIO_voidSetPinOutputLevel(port, pin, GPIO_OutputLevel_High);
	#endif
}
...
void GPIO_voidSetPinOutputSpeed(GPIO_PortName_t port, u8 pin, GPIO_OutputSpeed_t outputSpeed)
{
	...
	if (pin > 7)
	{
		...
		crIndex = 0;
	EDT_REG(GPIO[port]->CR[crIndex], 4*pin, outputSpeed, 2);
	#endif
}
