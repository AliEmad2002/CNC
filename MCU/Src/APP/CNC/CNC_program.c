	...
	CNC_t* CNC, GPIO_Pin_t _autoLevelingProbePin, Stepper_t* stepperArr)
{
	 ...
	 * the function "STK_u32GetTicksPerSecond()" every time.
	 */
...
void CNC_voidExecuteRestoreSavedAutoLevelingData(CNC_t* CNC)
{
	...
	CNC->autoLevelingGridxN =
		FPEC_u32ReadHalfWord(FPEC_HALF_WORD_ADDRESS(55, 0));
	CNC->autoLevelingGridyN =
		FPEC_u32ReadHalfWord(FPEC_HALF_WORD_ADDRESS(55, 1));
	...
	)
{
	...
	/*
	 * to compensate the precision error resulted from the modulus operation,
	 * "stepsOfMostToStepDash[]" and "stepsOfSmallerPerMostStepsDash[]" are
	 * implemented.
	 */
	u32 stepsOfMostToStepDash[2];
	u32 stepsOfSmallerPerMostStepsDash[2];
	...
	else
	{
		...
		Math_voidFindSimplistForm(
			displacementsAbs[axisSorted[0]], displacementsAbs[axisSorted[1]],
			&stepsOfMostToStepDash[0], &stepsOfSmallerPerMostStepsDash[0]
			);
	}
	...
	else
	{
		...
		Math_voidFindSimplistForm(
			displacementsAbs[axisSorted[0]], displacementsAbs[axisSorted[2]],
			&stepsOfMostToStepDash[1], &stepsOfSmallerPerMostStepsDash[1]
			);
	}
			...
			for (u8 i = 0; i < 2; i++)
			{
				...
				if (dDoneMost % stepsOfMostToStep[i] == 0)
				{
					...
					if (dDone[i] < dTotal[i])
					{
						 ...
						 * more than "stepsOfSmallerPerMostStepsDash[i]" steps
						 * per every "stepsOfMostToStepDash[i]" steps the most
						 * makes
						 */
						if (dDone[i] % stepsOfSmallerPerMostStepsDash[i] == 0)
						{
							if (dDoneMost % stepsOfMostToStepDash[i] != 0)
								continue;
						}
						Stepper_voidStep(
